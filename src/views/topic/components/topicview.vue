<template>
  <div>
    <mt-header :title="topicObj.topicTitle">
    ...
    <div class="bg-f heads bbw1 p10">
      <h3 style="font-size:18px;">{{ topicObj.topicTitle }}</h3>
      <dl class="itemlist">
    ...
    <div class="bg-f bbw1 p12">
      ...
      <div class="display">
        <div class="message" v-html="topicObj.topicContent">
        </div>
      </div>
...
export default {
  ...
  methods: {
    load() {
      ...
      findTopic({topicId: id}).then(res => {
        this.topicObj = res.obj
      })
    },
    changeType () {
    },
